...
def clear_domains_industries_table!
  DomainIndustry.all.each do |domain_industry|
    $rom_container.commands[:domains][:delete].find(domain_industry.id).call
  end
end
  
def clear_tmp_industries_table!
  TmpIndustry.all.each do |tmp_industry|
    $rom_container.commands[:tmp_industries][:delete].find(tmp_industry.id).call
  end
